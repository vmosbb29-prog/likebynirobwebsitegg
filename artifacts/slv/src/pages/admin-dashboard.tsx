...
interface KeyRow { key: string; keyType: string; expiresAt: string; likeUsed: boolean; visitUsed: boolean; usedCount: number; useLimit: number | null; dailyUseLimit: number | null; dailyUseCount: number; dailyUseResetAt: string | null; createdAt: string; }
...
export default function AdminDashboard() {
  ...
  const [newKeyDays, setNewKeyDays] = useState(30); const [newKeyLimit, setNewKeyLimit] = useState<number | "">("");  const [newKeyDailyLimit, setNewKeyDailyLimit] = useState<number | "">("");  const [newKeyCustom, setNewKeyCustom] = useState(""); const [newKeyType, setNewKeyType] = useState<"like"|"visit"|"both">("both");
  ...
  async function createKey(e: React.FormEvent) {
    e.preventDefault();
    const r = await adminFetch("/api/admin/keys", { method: "POST", body: JSON.stringify({ validityDays: newKeyDays, useLimit: newKeyLimit === "" ? null : Number(newKeyLimit), dailyUseLimit: newKeyDailyLimit === "" ? null : Number(newKeyDailyLimit), customKey: newKeyCustom.trim() || undefined, keyType: newKeyType }) });
          ...
          <div className="card-glass rounded-2xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-blue-900/20 text-slate-500">{["Key","Type","Expires","Used","Total Limit","Daily Today","Daily Limit","Action"].map(h => <th key={h} className="p-3 text-left font-medium whitespace-nowrap">{h}</th>)}</tr></thead>
                <tbody className="divide-y divide-blue-900/10">
                  {keys.map(k => {
                    ...
                    const typeLabel = k.keyType === "like" ? { text: "👍 Like", cls: "bg-pink-900/30 text-pink-300 border-pink-800/40" } : k.keyType === "visit" ? { text: "👁 Visit", cls: "bg-purple-900/30 text-purple-300 border-purple-800/40" } : { text: "👍👁 Both", cls: "bg-blue-900/30 text-blue-300 border-blue-800/40" };
                    return (
                    <tr key={k.key} className="hover:bg-white/2">
                      <td className="p-3 font-mono text-blue-300">{k.key}</td>
                      <td className="p-3"><span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${typeLabel.cls}`}>{typeLabel.text}</span></td>
                  ...
                  {keys.length === 0 && <tr><td colSpan={8} className="p-6 text-center text-slate-600">No keys</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
